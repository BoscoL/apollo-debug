...
public class ApolloApplication {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (commonContext.getEnvironment().containsProperty("configservice")) {
      String port = commonContext.getEnvironment().getProperty("configservice.port");

      ConfigurableApplicationContext configContext =
          new SpringApplicationBuilder(ConfigServiceApplication.class).parent(commonContext)
              .sources(RefreshScope.class).run(StringUtils.isBlank(port) ? args: ArrayUtils.add(args, "--server.port=" + port));
    ...
    if (commonContext.getEnvironment().containsProperty("adminservice")) {
      String port = commonContext.getEnvironment().getProperty("adminservice.port");
